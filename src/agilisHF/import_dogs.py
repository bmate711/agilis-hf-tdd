...
def import_data(raw_dog_list, db_context):
    validate_dog_list(raw_dog_list)
    dogs = []
    for dog in raw_dog_list:
        dogs.append(Dog(**dog).to_bson())
    db_context.insert_many(dogs)
    return True


def validate_dog_list(raw_dog_list):
    for dog in raw_dog_list:
